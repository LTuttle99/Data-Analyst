...
class BookOfBusinessAnalyzer:
    ...
    def _classify_business_type(self, value: str) -> str:
        ...
        # Order matters: check renewal first so "new" does not false-hit "renew"
    ...
    def infer_schema(self) -> dict:
        ...
        for col in columns:
            ...
            if not schema["agency_code"] and any(
                kw in col_lower for kw in ["agency", "agent code", "agency code", "producer code", "office code"]
            ):
            ...
            if not schema["client_id"] and any(
                kw in col_lower for kw in ["id", "client", "customer", "account", "name"]
            ):
    ...
    def _compute_forecast_outlook(
        ...
        """Creates the executive 'based on what we have now, this is where we will be' forecast."""
        if monthly_df is None or monthly_df.empty or target_series not in monthly_df.columns:
            return self._empty_forecast_outlook(projection_target)

        forecast_df = monthly_df.copy()
        forecast_df = forecast_df.sort_values("YearMonth").reset_index(drop=True)
        ...
        current_actual = self._safe_float(current_year_df[target_series].sum())
        previous_year_actual = self._safe_float(previous_year_df[target_series].sum())

        last_period = forecast_df["YearMonth"].max()
        last_month_number = int(last_period.month)
        remaining_months = max(0, 12 - last_month_number)
        ...
        if avg_monthly != 0:
            volatility_ratio = abs(residual_std / avg_monthly)
        else:
        ...
        if goal_value and goal_value > 0:
            ...
            if projected_gap_to_goal >= 0:
                goal_status = "Projected Above Goal"
            else:
                goal_status = "Projected Below Goal"
        ...
        # Confidence score is intentionally conservative.
        # It rewards more history and stronger fit while penalizing volatility.
    ...
    def run_analysis(
        ...
        # Compute full New vs Renewal split before view filter
        ...
        # Split monthly timeline by business type
        ...
        # 9. Existing next-12-month projections
        projections = []
        ...
        # 10. New executive forecast outlook
